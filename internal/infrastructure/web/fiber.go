...
import (
	"context"
	"github.com/deploydesexta/cccat12/internal/infrastructure/jsonutil"
...
func (r *FiberRouterAdapter) Router(ctrl Router) {
	ctrl.Bind(r)
}

func (r *FiberRouterAdapter) Get(path string, handler HandlerFunc) {
	r.Fiber.Get(path, func(c *fiber.Ctx) error {
		return handler(&FiberRequestAdapter{c})
	})
}

func (r *FiberRouterAdapter) Post(path string, handler HandlerFunc) {
