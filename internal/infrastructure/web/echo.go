...
func (r *EchoRouterAdapter) Router(ctrl Router) {
	ctrl.Bind(r)
}

func (r *EchoRouterAdapter) Get(path string, handler HandlerFunc) {
	r.Echo.GET(path, func(c echo.Context) error {
		return handler(&EchoRequestAdapter{c})
	})
}

func (r *EchoRouterAdapter) Post(path string, handler HandlerFunc) {
